...
import { edgeHealth, edgeNotifications, examSchedules, participants, storageHealth, syncBatches } from '@sentraujian/fixtures';
...
function NotificationView() { return <section className="panel"><div className="title"><div><small>In-App Notification Edge</small><h2>Operational events</h2></div><button>Mark all read</button></div><div className="notification-list">{edgeNotifications.map((notification) => <article className={`notification ${notification.severity.toLowerCase()}`} key={notification.id}><div><strong>{notification.title}</strong><p>{notification.message}</p><small>{notification.createdAt}</small></div><span className="state">{notification.read ? 'READ' : 'UNREAD'}</span></article>)}</div></section>; }
function StorageView() { return <section className="panel"><div className="title"><div><small>Local package cache</small><h2>Storage capacity</h2></div><span className="state active">{storageHealth.status}</span></div><div className="storage-meter"><div style={{ width: `${storageHealth.usedPercent}%` }} /></div><div className="storage-stats"><strong>{storageHealth.usedPercent}% used</strong><span>{storageHealth.availableGb} GB available</span></div><p>Warning at {storageHealth.warningPercent}% · Critical at {storageHealth.criticalPercent}%</p></section>; }
